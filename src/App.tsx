...
const translations = {
  en: {
    ...
    unverifiedMessage: "Your account is not verified. Please verify your email before using the app.",
  ...
  de: {
    ...
    unverifiedMessage: "Ihr Konto ist nicht verifiziert. Bitte verifizieren Sie Ihre E-Mail, bevor Sie die App nutzen.",
    invalidCredentials: "Ungültiger Benutzername oder Passwort.",
    userAlreadyExists: "Benutzername existiert bereits.",
    simulateVerify: "E-Mail-Verifizierung simulieren",
  ...
  vi: {
    ...
    unverifiedMessage: "Tài khoản của bạn chưa được xác minh. Vui lòng xác minh email trước khi sử dụng.",
    invalidCredentials: "Tên đăng nhập hoặc mật khẩu không đúng.",
    userAlreadyExists: "Tên đăng nhập đã tồn tại.",
    simulateVerify: "Mô phỏng xác minh Email",
    manageUsers: "Quản lý người dùng",
    makeAdmin: "Chỉ định Admin",
    removeAdmin: "Gỡ bỏ Admin",
...
const AuthComponent: React.FC<AuthProps> = ({ users, setUsers, setCurrentUser, t, isDarkMode, buttonColor }) => {
  ...
  const handleRegister = (e: React.FormEvent) => {
    ...
    if (users.find(u => u.username === username)) {
      setMessage(t.userAlreadyExists);
      return;
    }
    const newUser: User = { username, password, isAdmin: false, isVerified: false };
  ...
  const handleVerifyEmail = () => {
    const updatedUsers = users.map(u => u.username === username ? { ...u, isVerified: true } : u);
  ...
  return (
    <div className={`min-h-screen flex items-center justify-center ${isDarkMode ? 'bg-gray-900 text-white' : 'bg-white text-black'}`}>
      <div className={`p-8 rounded-lg shadow-xl w-full max-w-md ${isDarkMode ? 'bg-gray-800' : 'bg-gray-50'}`}>
        <h2 className="text-3xl font-bold mb-6 text-center">{isLogin ? t.login : t.register}</h2>

        {message && (
          <div className={`p-3 rounded mb-4 ${message === t.registrationSuccessful || message === t.verificationSuccessful ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}>
            {message}
          </div>
        )}
        ...
        <form onSubmit={isLogin ? handleLogin : handleRegister} className="space-y-4">
          <div>
            <label className="block mb-1 font-semibold">{t.username}</label>
            <input
              type="email"
              required
              value={username}
          ...
          <button
            type="submit"
            className="w-full p-3 rounded text-white font-bold text-lg transition hover:opacity-90"
        ...
        {message === t.unverifiedMessage && (
          <button
            onClick={handleVerifyEmail}
            className="w-full mt-4 p-3 rounded bg-blue-500 text-white font-bold"
          >
            {t.simulateVerify}
          </button>
        )}
...
const ExpenseTracker: React.FC = () => {
  ...
  useEffect(() => {
    const savedUsers = localStorage.getItem("expense_tracker_users");
    const savedCurrentUser = localStorage.getItem("expense_tracker_current_user");

    const parsedUsers: User[] = savedUsers ? JSON.parse(savedUsers) : [];

    // Initialize admin if not exists
    const adminExists = parsedUsers.find(u => u.username === "dotrung1998@gmail.com");
    if (!adminExists) {
      parsedUsers.push({
        username: "dotrung1998@gmail.com",
        password: "123456",
        isAdmin: true,
        isVerified: true
      });
      localStorage.setItem("expense_tracker_users", JSON.stringify(parsedUsers));
    }
  ...
  useEffect(() => {
    if (currentUser) {
      localStorage.setItem("expense_tracker_current_user", JSON.stringify(currentUser));
    } else {
  ...
  const SettingsModal = () => {
    ...
    const [innerSettingsTab, setInnerSettingsTab] = useState<"appearance" | "categories" | "users">(settingsTab === "categories" ? "categories" : "appearance");

    const toggleAdmin = (username: string) => {
      setUsers(prev => prev.map(u => u.username === username ? { ...u, isAdmin: !u.isAdmin } : u));
    };
      ...
      <div className={`fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50`}>
        <div className={`${isDarkMode ? 'bg-gray-800' : 'bg-white'} rounded-lg p-6 max-w-3xl w-full max-h-[85vh] flex flex-col`}>
          ...
          <div className="flex gap-2 mb-4 border-b border-gray-300">
            <button
              onClick={() => setInnerSettingsTab("appearance")}
              className={`px-4 py-2 font-semibold transition ${
                innerSettingsTab === "appearance"
            ...
            <button
              onClick={() => setInnerSettingsTab("categories")}
              className={`px-4 py-2 font-semibold transition ${
                innerSettingsTab === "categories"
                  ? `border-b-2 ${isDarkMode ? 'border-blue-400 text-blue-400' : 'border-blue-500 text-blue-500'}`
                  : 'opacity-60'
              }`}
            >
            ...
            {currentUser?.isAdmin && (
              <button
                onClick={() => setInnerSettingsTab("users")}
                className={`px-4 py-2 font-semibold transition ${
                  innerSettingsTab === "users"
                    ? `border-b-2 ${isDarkMode ? 'border-blue-400 text-blue-400' : 'border-blue-500 text-blue-500'}`
                    : 'opacity-60'
                }`}
              >
          ...
          <div ref={modalScrollRef} className="overflow-y-auto flex-1">
            ...
            {innerSettingsTab === "users" && currentUser?.isAdmin && (
              <div className="space-y-4">
                <h3 className="text-xl font-bold mb-4">{t.manageUsers}</h3>
                <div className="space-y-3">
                  {users.map(user => (
                    <div key={user.username} className={`p-4 rounded flex items-center justify-between ${isDarkMode ? 'bg-gray-700' : 'bg-gray-100'}`}>
                      <div>
                        <div className="font-bold">{user.username}</div>
                        <div className="text-sm opacity-75">
                          {user.isAdmin ? t.manageUsers : "User"} • {user.isVerified ? t.verified : t.unverified}
                        </div>
                      </div>
                      {user.username !== currentUser.username && (
                        <button
                          ...
                          style={{ backgroundColor: user.isAdmin ? '#E74C3C' : '#2ECC71' }}
                        >
        ...
        <div className="flex justify-between items-center mb-6">
          ...
          <div className="flex gap-2 items-center">
            ...
            <button
              ...
              className="p-2 rounded bg-red-500 text-white font-bold transition hover:bg-red-600"
              title={t.logout}
            >
              <X size={20} />
